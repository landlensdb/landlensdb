    ...
    for attempt in range(retries):
        try:
            ...
            graph = ox.graph_from_bbox(
                bbox=bbox_tuple, network_type=network_type, truncate_by_edge=True
            )
            network = ox.graph_to_gdfs(graph, nodes=False)
            return network
        except Exception as e:
            if attempt == retries - 1:
                msg = f"Failed to fetch OSM network after {retries} attempts: {str(e)}"
...
def optimize_network_for_snapping(network, simplify=True, remove_isolated=True):
    ...
    if remove_isolated:
        ...
        network = network[
            network.geometry.apply(lambda g: any(c in largest_cc for c in g.coords))
        ]
...
def validate_network_topology(network):
    ...
    report = {"original_size": len(network), "issues": [], "repairs": []}
    ...
    if len(components) > 1:
        ...
        report["repairs"].append(
            "Consider using optimize_network_for_snapping() to clean"
        )
