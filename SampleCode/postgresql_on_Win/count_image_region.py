...
def Mapillary4_SeqIDs_fromRegion(
    access_token, west, south, east, north, zoom_level, start_date, end_date
):
    ...
    for tile in tiles:
        ...
        for feature in data["features"]:
            ...
            timeJP = datetime.datetime.fromtimestamp(
                int(unixtime) / 1000, datetime.timezone(datetime.timedelta(hours=9))
            ).timestamp()
...
def main():
    ...
    allIMGIDs = []
    for SEQID in SEQIDs:
        IMGIDs = Mapillary4_seqID2ImageIDs(access_token=token, seqID=SEQID)
