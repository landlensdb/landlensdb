...
def Mapillary4_imageID2json(access_token, imageID, export_path, export_img=True):
    ...
    url = "https://graph.mapillary.com/{}?fields=id,sequence,camera_type,captured_at,compass_angle,computed_compass_angle,sequence,geometry,computed_geometry,thumb_1024_url&access_token={}".format(
        imageID, access_token
    )
    ...
    if export_img and image_url is not None:
        ...
        with open(
            "{}/{}/image_{}.jpg".format(export_path, sequence_id, imageID), "wb"
        ) as handler:
...
def Mapillary4_seqID2ImageIDs(access_token, seqID):
    ...
    url = "https://graph.mapillary.com/image_ids?sequence_id={}&access_token={}".format(
        seqID, access_token
    )
    headers = {"Authorization": "OAuth {}".format(access_token)}
    response = requests.get(url, headers=headers)
...
def Mapillary4_insertDB(access_token, imageID, export_path, export_img):
    ...
    JSON_data = Mapillary4_imageID2json(
    ...
    # insert a data
    cur.execute(
        "INSERT INTO mly_table (imageid, sequence, unixtime, type, angle, jsonb_data, image, location) VALUES  (%s, %s, %s, %s, %s, %s, %s, %s) ON CONFLICT DO NOTHING",
        [IMAGE_ID, SEQ_ID, UNIXTIME, CTYPE, ANGLE, json.dumps(JSON_data), PIC, LOC],
    )
...
def main():
    ...
    IMGID = Mapillary4_seqID2ImageIDs(
        access_token=token,
    ...
    for ID in IMGID:
        out = Mapillary4_insertDB(
            access_token=token, imageID=ID, export_path="/config/temp", export_img=True
        )
